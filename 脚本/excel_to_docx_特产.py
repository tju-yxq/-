...
TEMPLATE_PATH = r"模板\特产模板.docx"   # Word 模板文件路径
OUTPUT_DIR = "生成结果/特产"           # 输出文件夹
SHEET_NAME = "特产"                 # 读取的工作表名；如果为 None，则默认读取第一个表
...
def replace_placeholders(doc: Document, row_data: dict) -> None:
    ...
    # --- 增加字段别名兼容处理 ---
    mapping["{特产简介}"] = mapping.get("{特产介绍}", "无")
...
def main() -> None:
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        shop_name = safe_str(row_data.get('特产名称', ''))
