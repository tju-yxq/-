...
TEMPLATE_PATH = r"模板\景点模板.docx"   # Word 模板文件路径
OUTPUT_DIR = "生成结果/景点"           # 输出文件夹
SHEET_NAME = "景点"                 # 读取的工作表名；如果为 None，则默认读取第一个表
...
def replace_placeholders(doc: Document, row_data: dict) -> None:
    ...
    # --- 增加字段别名兼容处理 ---
    mapping["{景点类型}"] = mapping.get("{景点类型及其目的}", "无")
    mapping["{景区内饮用水提供、餐饮场所}"] = mapping.get("{景区内有无饮用水提供、有无餐饮场所}", "无")
    mapping["{节假日，特殊人群门票优惠}"] = mapping.get("{节假日，特殊人群是否有门票优惠}", "无")
    # Excel 可能会有多个联系人的记录，这里合并或取其中一个
    mapping["{联系人}"] = mapping.get("{联系人（如景区经理）}", mapping.get("{联系人身份及称谓}", "无"))
...
def main() -> None:
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        shop_name = safe_str(row_data.get('景点名称', ''))
